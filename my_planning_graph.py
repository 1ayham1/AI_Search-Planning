...
class PlanningGraph():
    ...
    '''
        SOME NOTES (ref. Ch 11.4 AIMD second edition)
        _____________________________________________
        1. A literal that does not appear in the final level of the graph cannot be achieved by any plan.
        2. The Concept of level Cost of the Goal:
                - We can estimate the cost of achieving any goal literal as the level at
                  which it first appears in the planning graph.
                - Such estimates are admissible for the individual goals
                - Serial Planning Graphs might be utilized if the estimate is not good if
                  planning graphs allow multiple actions
        3. Serial Graphs insists that only one action can actually occur at any given time state_map
                - This is done by adding mutex links between every pair of actions Exception
                  Persistence actions.
        4. Three Approaches to estimate the cost of a conjunction of goals:
                a- The MAX-LEVEL heuristic:
                        * takes max. level cost of any of the goals.
                        * admissible but not necessarily very accurate.
                b- The LEVEL-SUM heuristic:
                        * follow subgoal indepenence assumption, retuns the sum of the level costs of
                          the goal.
                        * inadmissible but works in practice
                c- The SET-LEVEL heuristic:
                        * finds the level at which all the literals in the conjunctive goal
                          appear in the planning graph without any pair of them being mutually
                          exclusive
        5. The GraphPlan algorithm:
                - Check whether all goal literals are present in the currentl level with
                  no mutex links between any pair of them
                - Expands the graph by adding the actions for current level and the state literals  for
                  the next level, if the solution was not found.
                - Repeat until a solution is found or it is learned that no solution exists.
    ...
    def add_action_level(self, level):
        ...
        '''
        LOGIC:
            - Select candidate actions per level with satisfied preconditions
            - Establish parent-child pointers among admissible actions and the
              corrosponding states.
        '''
        self.a_levels.append(set())  # A0 set of action nodes

        for action in self.all_actions:

            action_node = PgNode_a(action)  #proposed action node

            admissible_action = action_node.prenodes.issubset(self.s_levels[level])
            # TRUE if the preconditions are already satisfied by previous state

            if admissible_action:
    ...
    def add_literal_level(self, level):
        ...
        '''
        LOGIC:
            - Iterate through all action-nodes in the previous action levle.
            - for each of those nodes get their corrosponding effect nodes and
            add them to the the state node.
            - Establish parent-child relation among action and effect nodes.
        ...
        for action in prev_Action_Level:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        '''
        LOGIC:
            Pair of actions A1 & A2 are inconsistent if inter-positive effect canceled
            the inter-negative one. i.e. the simultanous occurance of X or O as shown below
        ...
        A1_pos = node_a1.action.effect_add
        A1_neg = node_a1.action.effect_rem

        A2_pos = node_a2.action.effect_add
        A2_neg = node_a2.action.effect_rem

        inconsistency_1 = [effect for effect in A1_pos if effect in A2_neg]
        inconsistency_2 = [effect for effect in A2_pos if effect in A1_neg]

        inconsistent_effect = inconsistency_1 or inconsistency_2
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        '''
        LOGIC:
            Enumerating possibilities of the effect of one action is the negation of
            a precondition of the other:


                                             |   A1_pos   |  A2_pos    |
                            --------------------------------------------
                            precond_A1_neg   |            |   mutex_1  |
                            precond_A2_neg   |   mutex_1  |            |


                                             |   A1_neg  |   A2_neg   |
                            -------------------------------------------
                            precond_A1_pos   |           |   mutex_3  |
                            precond_A2_pos   |  mutex_4  |            |
        ...
        pA1_pos = node_a1.action.precond_pos
        pA1_neg = node_a1.action.precond_neg

        pA2_pos = node_a2.action.precond_pos
        pA2_neg = node_a2.action.precond_neg


        mutex_1 = [effect for effect in A2_pos if effect in pA1_neg]
        mutex_2 = [effect for effect in A1_pos if effect in pA2_neg]

        mutex_3 = [effect for effect in A2_neg if effect in pA1_pos]
        mutex_4 = [effect for effect in A1_neg if effect in pA2_pos]


        mutual_exclusive = mutex_1 or mutex_2 or mutex_3 or mutex_4
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for pA1 in node_a1.parents:
            for pA2 in node_a2.parents:
                if (pA1.is_mutex(pA2)):
                    return True
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        negated_nodes = xor(node_s1.is_pos, node_s2.is_pos) # one is pos and the other is negtive
        equal_literals = (node_s1.symbol == node_s2.symbol)

        return (equal_literals and negated_nodes)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for state_1 in node_s1.parents:
            for state_2 in node_s2.parents:

                if not (state_1.is_mutex(state_2)):
                    return False

        return True #no actions could achieve the literals at the same time
    ...
    def h_levelsum(self) -> int:
        ...
        for each goal in the problem, determine the level cost, then add them together
            ...
            * Do that :point_up: for each goal
            * Add up all those numbers
            
        #Ref: https://discussions.udacity.com/t/understand-level-sum-heuristic/225706/3
        '''
